...
class Box(models.Model):
    ...
    Represents a storage box with hierarchical relationships and metadata.
    ...
    Attributes:
        ...
        name (CharField): Name of the box, required and unique.
    ...
    Meta:
        ...
        constraints: Ensures that the name is unique.
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["name"], name="unique box")]
...
class File(models.Model):
    """
    Represents a file entity in the system, which can be associated with an item or a box.
    ...
    Meta:
        constraints (list): Ensures that the combination of name, item, and box is unique.
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["name", "item", "box"], name="unique file")]
...
class URL(models.Model):
    """
        Represents a URL resource associated with an Item or Box.

    Attributes:
        id (UUID): Primary key, unique identifier for the URL.
        slug (str): Optional unique slug for the URL.
        created_by (User): User who created the URL entry.
        created_at (datetime): Timestamp when the URL was created.
        updated_by (User): User who last updated the URL entry.
        updated_at (datetime): Timestamp when the URL was last updated.
        description (str): Optional description of the URL.
        name (str): Name or label for the URL.
        url (str): The actual URL.
        item (Item): Optional reference to an associated Item.
        box (Box): Optional reference to an associated Box.
        is_active (bool): Indicates if the URL is active.
        is_deleted (bool): Indicates if the URL is marked as deleted.

    Meta:
        Unique constraint on the combination of name, item, and box.

    Methods:
        __str__(): Returns the URL as its string representation.
    """
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["name", "item", "box"], name="unique url")]
